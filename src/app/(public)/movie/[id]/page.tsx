...
import CastCard, { ICastCard } from '@/components/MovieHomepage/CastCard';
...
async function getMovieDetails(id: IMovieCard['id']) {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${id}?api_key=${process.env.NEXT_API_KEY}&language=en-US`
  );
  return res.json();
}
...
async function getMovieRec(id: IMovieCard['id']) {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${id}/recommendations?api_key=${process.env.NEXT_API_KEY}&language=en-US`
  );
  return res.json();
}
...
async function getMovieCasts(id: IMovieCard['id']) {
  const res = await fetch(
    `https://api.themoviedb.org/3/movie/${id}/credits?api_key=${process.env.NEXT_API_KEY}&language=en-US`
  );
  return res.json();
}
...
const MovieDetailsPage = () => {
  const { id } = useParams();
  const { details, loading } = useDetails('movie', id);
